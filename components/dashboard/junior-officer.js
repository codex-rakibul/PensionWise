...
import { updateJuniorOfficerStatus } from '@/app/feature/pensionData/pensionFormSlice';

function JuniorOfficer() {
    ...
    // Match User Id
    const matchingData = allJuniorOfficer.alljuniorOfficerData.find((user) => user.userId === localStorage_user_id)
    // console.log("-----------", matchingData);

    // Accepts Action
    const handleAcceptForm = (userId) => {
        // Dispatch the action to update the form_status
        dispatch(updateJuniorOfficerStatus({ userId, status: "success" }));
  ...
  .filter((formData) => formData?.apply_status === "apply" && formData?.form_status.junior_officer === "waitting") // Filter by apply_status
  .map((formData, index) => {
    return {
      id: index,
